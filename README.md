...
- `rank-shots/` — final stage. Loads the Transcribe JSON (if any), slices spoken words per shot, sends one Bedrock Converse call to the chosen text model with all shot descriptions + their dialogue, then takes the top-K by score. Each kept segment is stamped with a `rank` field (1 = strongest match). Title stays clean — chip labels show only the title (or "Custom" for empty-title custom segments); AI rank is surfaced as an inspector badge so it doesn't get confused with reel-order position.
...
- `highlightEditorModal.js` + `editorTracks.js` — two-mode editor with a `[ Source │ Reel · 3:42 ]` toggle:
   - **Source mode** — chips at their absolute time on the original video, with a time ruler and a draggable red playhead. Drag a chip body to move on source-time; drag the L/R trim handles to trim. Click any part of the chip (body or handles) to select + preview. Snap-to-playhead and snap-to-neighbor at 5 px tolerance, 1 s minimum-duration floor.
   - **Reel mode** — chips laid out left-to-right in playback order, widths proportional to duration. Drag a chip horizontally to reorder; click any chip or anywhere on the ribbon to jump the reel playhead to that point; drag the playhead to scrub the reel. Auto-advances through segments when playing.
  Native `<video>` scrubber is hidden via CSS — the active timeline is the one true scrub surface. Spacebar plays the active mode; ←/→ steps ±1 frame at 25 fps (Shift = ±1 s). `preload="auto"` so the proxy MP4 starts streaming as soon as the modal opens. Drag/resize gestures use [interact.js](https://interactjs.io/) (vendored under `webapp/third_party/interactjs-bundle/`).
- `outputTab.js` — the **Video Edit** tab. Detection form with two model dropdowns (Video describer / Rank model), both with empty placeholders; submit stays disabled until both are picked. Final action is **Export**.
