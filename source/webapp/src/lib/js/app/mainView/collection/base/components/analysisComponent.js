...
// GenAI
import GenAITab from './analysis/genai/genaiTab.js';
// AdBreak
  ...
  constructor(previewComponent) {
    ...
    types.forEach((type) => {
      ...
      datas.forEach((data) => {
        ...
        } else if (type === AdBreak) {
          controller = new AdBreakTab(previewComponent, data);
        } else if (type === Shoppable) {
          controller = new ShoppableTab(previewComponent, data);
        }
        if (controller) {
    ...
    types.forEach((type) => {
      ...
      datas.forEach((data) => {
        ...
        if (type === Celeb) {
          ...
          controller = new ImageCaptionTab(previewComponent, data);
        }
        if (controller) {
          this.$tabControllers.push(controller);
        }
